...
function M.run()
  ...
  if ft ~= "typescript" and ft ~= "typescriptreact" then
    vim.notify("CodePoints: only TypeScript files are supported (current filetype: " .. ft .. ")", vim.log.levels.WARN)
    return
  end

  local treesitter = require("code-points.treesitter")
  local entries = treesitter.get_code_points(source_bufnr)
  ...
  window.open(source_bufnr, entries)
end
